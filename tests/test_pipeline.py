...
class PipelineTest(unittest.TestCase):
    def test_append_new_trending_papers_deduplicates_by_paper_url(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            added = pipeline.append_new_trending_papers(index_path, papers, update_time="2026-07-07 16:13:59")

            lines = index_path.read_text(encoding="utf-8").splitlines()

        self.assertEqual(added, 1)
        self.assertEqual(len(lines), 2)
        self.assertIn("New Paper\thttps://huggingface.co/papers/2606.23051\thttps://arxiv.org/pdf/2606.23051.pdf\t2026-07-07 16:13:59", lines)
    ...
    def test_run_trending_paper_job_only_runs_trending(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
                ...
                with patch("tech_crawler.trending_paper.pipeline.crawler.fetch_papers", return_value=[trending_paper]) as fetch:
                    with patch("tech_crawler.trending_paper.pipeline.download_papers.download_paper", side_effect=download_side_effect) as download:
                        with patch("tech_crawler.trending_paper.pipeline.read_papers.summarize_pdf") as summarize:
                            result = pipeline.run_trending_paper_job(root, "2026-07-01", update_time="2026-07-07 16:13:59")
            ...
            self.assertEqual(result.summarized_count, 0)
            self.assertEqual(
                (root / "data" / "papers" / "trending" / "trending_paper.txt").read_text(encoding="utf-8"),
                "Trending Paper\thttps://huggingface.co/papers/2606.23050\thttps://arxiv.org/pdf/2606.23050.pdf\t2026-07-07 16:13:59\n",
            )
            self.assertEqual(fetch.call_count, 1)
            self.assertEqual(download.call_count, 1)
            self.assertEqual(summarize.call_count, 1)

            expected_trending_pdf = root / "data" / "papers" / "trending" / "2026-W27" / "Trending Paper.pdf"
            summarize.assert_called_once_with(expected_trending_pdf, delay_seconds=5, proxies={"http": None, "https": None})
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict("os.environ", {}, clear=True):
                with patch("tech_crawler.trending_paper.pipeline.crawler.fetch_papers", return_value=[daily_paper]) as fetch:
                    with patch("tech_crawler.trending_paper.pipeline.download_papers.download_paper", side_effect=download_side_effect) as download:
                        with patch("tech_crawler.trending_paper.pipeline.read_papers.summarize_pdf") as summarize:
                            result = pipeline.run_today_paper_job(root, "2026-07-01", update_time="2026-07-07 16:13:59")

            self.assertEqual(result.trending_downloaded_count, 0)
            self.assertEqual(result.trending_summarized_count, 0)
            self.assertEqual(result.downloaded_count, 1)
            self.assertEqual(result.summarized_count, 1)
            self.assertTrue((root / "data" / "papers" / "20260701" / "paper.txt").exists())
            self.assertEqual(
                (root / "data" / "papers" / "20260701" / "paper.txt").read_text(encoding="utf-8"),
                "Daily Paper\thttps://huggingface.co/papers/2606.23051\thttps://arxiv.org/pdf/2606.23051.pdf\t2026-07-07 16:13:59\n",
            )
            self.assertEqual(fetch.call_count, 1)
            self.assertEqual(download.call_count, 1)
            self.assertEqual(summarize.call_count, 1)
            ...
            summarize.assert_called_once_with(expected_daily_pdf, delay_seconds=5, proxies={"http": None, "https": None})
